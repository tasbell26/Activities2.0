...
data.forEach(obj => console.log(obj.address.city))

// create an array of latitude and longitude coordinate pairs
const cord = [];
data.forEach(position => (
  let lat = position.address.geo.lat;
  let lng = position.address.geo.lng;
  cord.push(lat, lng);
))
console.log(cord);
...
const phrase = [];
data.forEach(obj => (
  let cP = `${obj.company.catchPhrase}, ${obj.company.bs}`
  phrase.push(cP);
))
